...
### Overview Text
```tsx
overview="When someone buys your product, Fabrk uses Stripe to handle everything securely. Customer clicks Buy, enters payment info on Stripe's checkout page..."
```
...
<DocsSection title="Understanding Webhooks">
  <DocsCallout variant="info" title="...">...</DocsCallout>
  <DocsCard title="WEBHOOK_EVENTS">
...
5. **Create DocsTable component** for key-value tables:
   ```tsx
   <DocsTable rows={[
     { key: "Successful payment", value: "4242 4242 4242 4242" },
   ]} />
   ```
