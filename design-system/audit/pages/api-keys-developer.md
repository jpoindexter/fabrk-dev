...
4. **Create typed API client**
   ...
   export const apiKeysApi = {
     list: (orgId: string) => fetch(`/api/api-keys?organizationId=${orgId}`).then(validateApiKeysResponse),
     create: (data: CreateKeyInput) => fetch('/api/api-keys', { method: 'POST', body: JSON.stringify(data) }).then(validateCreateResponse),
     revoke: (id: string) => fetch(`/api/api-keys/${id}`, { method: 'DELETE' }).then(validateDeleteResponse),
   };
   ```
