...
export default function WebhooksTutorialPage() {
  return (
    <FeatureGuideTemplate
      ...
      features={[
        { icon: Webhook, title: "22 Events", description: "Event types across 5 categories." },
        { icon: Shield, title: "HMAC-SHA256", description: "Signature verification for security." },
        { icon: RefreshCw, title: "Auto Retry", description: "Exponential backoff on failures." },
        { icon: Bell, title: "Tracking", description: "Delivery status monitoring." },
      ]}
      usage={[
        {
...
curl -X POST http://localhost:3000/api/test-webhook \\
  ...
  -d '{"event": "payment.succeeded"}'`,
      ...
      previous={{ title: "Stripe Payments", href: "/docs/tutorials/stripe-payments" }}
      ...
      <DocsSection title="Webhook Headers">
        <DocsCard title="HEADERS">
          <div className="space-y-1 font-mono text-sm text-muted-foreground leading-relaxed">
            ...
            <div>├─ <code className="bg-muted px-1 font-mono text-xs">X-Webhook-Retry</code> - Retry attempt number</div>
