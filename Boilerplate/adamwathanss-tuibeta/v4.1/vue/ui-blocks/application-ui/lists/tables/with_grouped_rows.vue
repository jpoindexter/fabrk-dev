            ...
            <tbody class="bg-gray-900">
              <template v-for="location in locations" :key="location.name">
                <tr class="border-t border-white/10">
                  <th colspan="5" scope="colgroup" class="bg-gray-800/50 py-2 pr-3 pl-4 text-left text-sm font-semibold text-white sm:pl-3">{{ location.name }}</th>
                </tr>
                <tr v-for="(person, personIdx) in location.people" :key="person.email" :class="[personIdx === 0 ? 'border-white/15' : 'border-white/10', 'border-t']">
                  <td class="py-4 pr-3 pl-4 text-sm font-medium whitespace-nowrap text-white sm:pl-3">{{ person.name }}</td>
