<template>
  <nav class="h-full overflow-y-auto" aria-label="Directory">
    <div v-for="letter in Object.keys(directory)" :key="letter" class="relative">
      <div class="sticky top-0 z-10 border-y border-t-white/5 border-b-white/10 bg-gray-900 px-3 py-1.5 text-sm/6 font-semibold text-white before:pointer-events-none before:absolute before:inset-0 before:bg-white/5">
        <h3 class="relative">{{ letter }}</h3>
      </div>
      <ul role="list" class="divide-y divide-white/5">
        <li v-for="person in directory[letter]" :key="person.email" class="flex gap-x-4 px-3 py-5">
          <img class="size-12 flex-none rounded-full bg-gray-800 outline -outline-offset-1 outline-white/10" :src="person.imageUrl" alt="" />
          <div class="min-w-0">
