 ...
 */
import { findComponent } from '../data/component-catalog.js';
...
function generateButton(name, variant) {
...
function generateCard(name, withHeader) {
...
function generateGenericComponent(name, component, withHeader) {
    const hexCode = generateHexCode(name);
...
function generateCustomComponent(name, withHeader) {
    const hexCode = generateHexCode(name);
    const title = name.toUpperCase().replace(/([A-Z])/g, '_$1').replace(/^_/, '');
