  ...
  return (
    <div className="bg-background flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardContent className="space-y-6 p-8">{/* Content */}</CardContent>
      </Card>
    </div>
  );
}
...
<div className="mb-8 flex justify-center">
  <div className="bg-warning/20 rounded-none p-6">
    <Wrench className="text-warning h-16 w-16" />
  </div>
</div>
```
...
```tsx
<div className="bg-warning/10 mb-8 inline-flex items-center gap-2 rounded-none px-6 py-4">
...
<div className="border-border mt-12 border-t pt-8">
  <p className="text-muted-foreground mb-2 text-sm">Need urgent assistance?</p>
  <a
...
```tsx
<UtilityPageTemplate centered maxWidth="2xl" showNav={false} showFooter={false}>
  {children}
</UtilityPageTemplate>
```
